...
public class HttpUtil {
    ...
    public static String getRetValGet(String url, Map<String, String>... params) {
        String retVal = null;
        try {
            ...
            /*for (Map<String, String> map : params) { //循环参数，有几项添加几项
				for (String key : map.keySet()) {
					if (map.get(key) != null && !"".equals(map.get(key))) { //如果参数为空则不传
						getMethod.addParameter(key, map.get(key));
					}
				}
			}*/
            getMethod.getParams().setParameter(HttpMethodParams.HTTP_CONTENT_CHARSET, "utf-8"); //统一编码UTF-8
